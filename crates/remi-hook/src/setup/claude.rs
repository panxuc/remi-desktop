//! Claude Code's part of `setup`, `uninstall` and `check`: the hooks remi needs, in the
//! `hooks` section of Claude Code's user settings file.
//!
//! remi's hooks are recognised by their command — a program named `remi-hook` running
//! `signal` — rather than by a marker, because Claude Code validates its settings and rejects a
//! hook entry carrying a key it doesn't know. The same rule picks up hooks written by hand
//! before `setup` existed, so the first `setup` replaces them instead of adding duplicates.
...
/// Where Claude Code keeps its settings, history and plugins, when not in `~/.claude`.
...
struct Hook {
    ...
    event: &'static str,
    /// What narrows the event: tool names, or for `Notification`, notification types.
    matcher: Option<&'static str>,
    signal: SignalEvent,
    /// Seconds Claude Code waits for the hook before carrying on without it.
    timeout: u64,
}
...
impl Hook {
    ...
    fn entry(&self, program: &str) -> HookEntry {
        HookEntry {
...
pub fn install(path: &Path, hook_path: &Path) -> Result<Edit, Error> {
    ...
    let added = add_hooks(&mut settings, &program).map_err(|what| Error::shape(path, what))?;
...
pub fn inspect(path: &Path, hook_path: &Path) -> Result<Inspection, Error> {
    ...
    let expected: Vec<HookEntry> = HOOKS.iter().map(|hook| hook.entry(&program)).collect();
